...
//登录窗口
function LoginWin(showType) {
    if (showType == null || showType == 0)
        BWShowWinAjax('.body', 'LoginWin', '/Home/Login/', null);
    else if (showType == 1)
        BWShowWinAjaxReload(null, '/Home/Login/', null);
}
//新增账号信息窗口
function NewEventWin() {
        BWShowWinAjax('.body', 'NewEventWin', '/Event/NewEvent/', null);
}
//登录表单绑定
function LoginForm() {
...
//注册窗口
function RegisterWin(showType) {
    if (showType == null || showType == 0)
        BWShowWinAjax('.body', 'RegisterWin', '/Home/Register/', null);
    else if (showType == 1)
        BWShowWinAjaxReload(null, '/Home/Register/', null);
}
//注册表单绑定
